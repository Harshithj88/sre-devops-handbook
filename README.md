# SRE / DevOps Handbook

## Overview

This repository is a practical SRE and DevOps handbook containing production-readiness checklists, troubleshooting runbooks, cheat sheets, architecture diagrams, incident response templates, and official documentation references.

The goal of this repository is to demonstrate hands-on knowledge of Site Reliability Engineering, cloud operations, Kubernetes, CI/CD, observability, incident response, automation, and production support practices.

the repo is inspired by practices from Google SRE Books, Microsoft Azure Well-Architected Framework, Kubernetes Documentation, Prometheus alerting rules, OpenTelemetry documentation, and the CNCF Landscape.

## Topics Covered

- Site Reliability Engineering
- DevOps Operations
- Kubernetes / AKS
- CI/CD
- GitHub Actions
- Azure DevOps
- Infrastructure as Code
- Observability
- Monitoring and Alerting
- Incident Response
- Change Management
- PowerShell Automation
- Production Readiness
- Postmortems
- SLOs, SLIs, and Error Budgets

## Repository Structure

| Folder | Purpose |
|---|---|
| `checklists/` | Production readiness, deployment, Kubernetes, and incident checklists |
| `cheatsheets/` | Quick command references for Kubernetes, Git, Azure CLI, Docker, PowerShell, and PromQL |
| `runbooks/` | Step-by-step troubleshooting guides for common production issues |
| `architecture/` | Architecture notes and diagrams |
| `docs/` | SRE/DevOps concepts and best practices |
| `resources/` | Official documentation and learning resources |
| `templates/` | Reusable templates for runbooks, postmortems, and change requests |
| `scripts/` | Automation examples |

## Why This Repo Exists

In real-world SRE and DevOps environments, reliability is not just about tools. It also depends on repeatable processes, clear documentation, incident response readiness, automation, and operational discipline.

This repo documents those practices in a structured, reusable format.

## Portfolio Highlights

This repository demonstrates:

- Ability to create operational documentation
- Knowledge of SRE best practices
- Kubernetes troubleshooting skills
- CI/CD and cloud platform understanding
- Incident response and postmortem process knowledge
- Production support mindset
- Automation-first approach

## Recommended Reading

- Google SRE Books
- Microsoft Azure Well-Architected Framework
- Kubernetes Documentation
- Prometheus Documentation
- Grafana Documentation
- OpenTelemetry Documentation
- CNCF Cloud Native Landscape